---
type: "source-file"
title: ".agents/skills/agent-ready-context/SKILL.md"
description: "Repository source file staged for OpenKB ingestion."
resource: ".agents/skills/agent-ready-context/SKILL.md"
source_path: ".agents/skills/agent-ready-context/SKILL.md"
source_kind: "markdown"
source_hash: "sha256:4d464be7ed3fd935e32e3359f09d1a521211fab019f325b9598c0fa5018842d0"
...
8. Initialize OpenKB when needed: create `okf/`, then run `openkb init` from inside it with explicit model/language from `references/openkb-providers.md`. Before the first LLM-backed command, give the disclosure from `references/privacy-and-data-flows.md`.
...
12. Run `openkb --kb-dir ./okf lint` as the OpenKB health check. Do not run `lint --fix`, `remove`, broad `recompile`, `query`, `visualize`, `watch`, `chat`, or Skill Factory commands without the consent rules in `references/openkb-lifecycle.md`.
...
Initialize OpenKB if `okf/` does not exist yet:
...
# references/openkb-lifecycle.md). Re-run --check after.
```
...
- Keep provider credentials in shell environment or gitignored `.env` files. `openkb init` may write `okf/.env`; keep it ignored. Credentials are hands-off: never ask for, read, print, validate, or write key values; state variable names and locations only, and never block agentification on a missing key (full rules in `references/openkb-providers.md`).
